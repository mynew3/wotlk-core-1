...
void WorldSession::HandleMovementOpcodes(WorldPacket & recvData)
{
	...
	if (movementAntiCheat(plrMover, movementInfo, mover, opcode)) {
    ...
    if (opcode == MSG_MOVE_FALL_LAND || opcode == MSG_MOVE_START_SWIM)
		...
		if (plrMover && !vehMover)                                            // nothing is charmed, or player charmed
		{
			...
			{
				case 617: underMapValueZ = 3.0f;
					break; // Dalaran Sewers
				case 618: underMapValueZ = 28.0f;
					break; // Ring of Valor
				default: underMapValueZ = -500.0f;
					break;
			}
			...
			{
				sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s produce %d anticheat alarms.", plrMover->GetName().c_str(), plrMover->m_anti_AlarmCount);
...
bool WorldSession::movementAntiCheat(Player* plrMover, MovementInfo& movementInfo, Unit* mover, uint16 opcode)
{
	// begin anti cheat
	bool check_passed = true;
#ifdef ANTICHEAT_DEBUG
	sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s > time: %d fall-time: %d | xyzo: %f, %f, %fo(%f) flags[%X] opcode[%s] | transport (xyzo): %f, %f, %fo(%f)",
	...
	sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s Transport > GUID: (low)%d - (high)%d",
		...
		if (World::GetEnableMvAnticheat() && !plrMover->IsGameMaster() && !plrMover->GetCharmerOrOwnerPlayerOrPlayerItself()->IsGameMaster() && !plrMover->GetCharmerOrOwnerPlayerOrPlayerItself()->GetVehicle())
		{
...
#ifdef ANTICHEAT_DEBUG
			sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s Time > cClientTimeDelta: %d, cServerTime: %d | deltaC: %d - deltaS: %d | SyncTime: %d", plrMover->GetName(), cClientTimeDelta, cServerTime, plrMover->m_anti_DeltaClientTime, plrMover->m_anti_DeltaServerTime, sync_time);
#endif
			...
			if (sync_time > GetMistimingDelta)
			{
				...
				if (bMistimingModulo)
				{
					...
					sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s, mistiming exception #%d, mistiming: %dms Action: %s", plrMover->GetName().c_str(), plrMover->m_anti_MistimingCount, sync_time, &Warden::Penalty);
#endif
					check_passed = false;
				}
				//if (vehMover)
			...
			if (!curDest)
			{
				...
				{
#ifdef ANTICHEAT_EXCEPTION_INFO
					sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s, AntiGravity exception. JumpHeight = %f, Allowed Vertical Speed = %f",
				...
				if (opcode == MSG_MOVE_JUMP)
				{
					if (no_fly_auras && no_swim_water)
					{
						if (plrMover->m_anti_JumpCount >= 1)
						{
							...
							check_passed = false;
#ifdef ANTICHEAT_EXCEPTION_INFO
							sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s, Multijump exception.", plrMover->GetName().c_str(), JumpHeight, plrMover->m_anti_Last_VSpeed);
#endif
							//if (vehMover)
							//    vehMover->Die();
							// Tell the player "Sure, you can fly!"
				...
				if (real_delta > allowed_delta)
				{
					...
					{
						sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s, speed exception | cDelta=%f aDelta=%f | cSpeed=%f lSpeed=%f deltaTime=%f", plrMover->GetName().c_str(), real_delta, allowed_delta, current_speed, plrMover->m_anti_Last_HSpeed, time_delta);
					}
					else {
						sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s, teleport exception | cDelta=%f aDelta=%f | cSpeed=%f lSpeed=%f deltaTime=%f", plrMover->GetName().c_str(), real_delta, allowed_delta, current_speed, plrMover->m_anti_Last_HSpeed, time_delta);
					}
#endif
					check_passed = false;
					//if (vehMover)
				...
				{
#ifdef ANTICHEAT_EXCEPTION_INFO
					sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s, mountain exception | tg_z=%f", plrMover->GetName().c_str(), tg_z);
#endif
					check_passed = false;
					//    if (vehMover)
					//        vehMover->Die();
				...
				if (no_fly_auras && !no_fly_flags)
				{
					...
					sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s, flight exception. {SPELL_AURA_FLY=[%X]} {SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED=[%X]} {SPELL_AURA_MOD_INCREASE_FLIGHT_SPEED=[%X]} {SPELL_AURA_MOD_MOUNTED_FLIGHT_SPEED_ALWAYS=[%X]} {SPELL_AURA_MOD_FLIGHT_SPEED_NOT_STACK=[%X]} {plrMover->GetVehicle()=[%X]}",
				...
				{
#ifdef ANTICHEAT_EXCEPTION_INFO
					sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s, waterwalk exception. [%X]{SPELL_AURA_WATER_WALK=[%X]}",
				...
				if (no_swim_in_water && movementInfo.pos.GetPositionZ() < 0.0001f && movementInfo.pos.GetPositionZ() > -0.0001f)
				{
					if (const Map *map = plrMover->GetMap())
					{
						...
						{
#ifdef ANTICHEAT_DEBUG
							sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s, teleport to plane exception. plane_z: %f", plrMover->GetName(), plane_z);
#endif
#ifdef ANTICHEAT_EXCEPTION_INFO
							if (plrMover->m_anti_TeleToPlane_Count > World::GetTeleportToPlaneAlarms())
							{
								sLog->outDebug(LOG_FILTER_WARDEN, "AC2-%s, teleport to plane exception. Exception count: %d", plrMover->GetName().c_str(), plrMover->m_anti_TeleToPlane_Count);
...
#ifdef ANTICHEAT_DEBUG
	sLog->outDebug(LOG_FILTER_WARDEN, , "%s CMSG_MOVE_KNOCK_BACK_ACK: time: %d, fall time: %d | xyzo: %f,%f,%fo(%f) flags[%X] Vspeed: %f, Hspeed: %f", GetPlayer()->GetName(), movementInfo.time, movementInfo.fallTime, movementInfo.pos.m_positionX, movementInfo.pos.m_positionY, movementInfo.pos.m_positionZ, movementInfo.pos.m_orientation, movementInfo.flags, movementInfo.jump.zspeed, movementInfo.jump.xyspeed);
#endif
